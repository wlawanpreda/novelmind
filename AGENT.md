...
| `image_provider.py` | route สร้างรูป (Imagen/local ComfyUI/hybrid) + auto-fallback |
| `gateway.py` | **HTTP service** ห่อ LLM+Image (FastAPI :9000) — คิว/lock/auth (ดู SERVICE_ARCHITECTURE.md) |
| `ansre_client.py` | SDK บางๆ เรียก gateway (`Ansre().llm()/.image()`) — ให้ client อื่นใช้ |
| `ansre` / `ansre.py` | CLI ประตูเดียว (wrapper + router) |
| `orchestrator.py` | ขับ pipeline ต่อเนื่อง (state-driven, lock, backpressure) |
| `scout.py` | ดึง trending novels + metric (rating/views/รีวิว/points) + popularity score/rank |
| `agent_analyzer.py` | วิเคราะห์ → Thai title, market score, จุดเด่น/hook/adapt-strategy |
| `trends.py` | สรุปเทรนด์จาก pool → คำแนะนำเรื่องต่อไป (ป้อนเข้า ideation) |
...
| `cover_generator.py` | สร้างปก — prompt ผ่าน llm_provider, รูปผ่าน image_provider (Imagen/local) |
...
- สร้างรูปใหม่ **ต้องผ่าน** `from image_provider import generate_image` (อย่าเรียก Imagen/ComfyUI ตรง)
- provider จะ route ผ่าน **gateway อัตโนมัติ** ถ้าตั้ง `ANSRE_GATEWAY_URL` (ล่ม→fallback ทำเองในเครื่อง); ไม่ตั้ง=import ตรงเหมือนเดิม
...
- `docs/MACMINI_SETUP.md` — ตั้ง Mac mini local LLM (Ollama)
- `docs/MACMINI_IMAGE_SETUP.md` — ตั้ง Mac mini สร้างรูปฟรี (ComfyUI/SDXL)
- `docs/SERVICE_ARCHITECTURE.md` — แยก LLM/Image เป็น gateway service (Phase 1 ทำแล้ว: gateway.py)
